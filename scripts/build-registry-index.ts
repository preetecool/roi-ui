...
`;
  ...
  // Write blocks data file
  const blocksDataContent = `// @generated
// This file is autogenerated by scripts/build-registry-index.ts
// Do not edit this file directly.
...
export const BlocksData: Record<string, BlockData> = ${JSON.stringify(blocksData, null, 2)};
`;

  const blocksDataOutputPath = path.join(process.cwd(), "registry/__blocks__.ts");
  await fs.writeFile(blocksDataOutputPath, blocksDataContent);
  console.log("✅ Built registry/__blocks__.ts (pre-computed block files)");

  // Format the generated files
  const { execSync } = await import("node:child_process");
  try {
    execSync("pnpm lint:fix registry/__index__.tsx registry/__loaders__.tsx registry/__blocks__.ts", {
      stdio: "inherit",
    });
