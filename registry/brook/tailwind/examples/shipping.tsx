      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-col gap-1">
            <div className="font-semibold text-foreground text-sm">{shipmentData.id}</div>
            <div className="text-muted-foreground text-xs">{shipmentData.driver}</div>
          </div>
          <div className="flex flex-col items-end gap-1">
            ...
            <div className="text-muted-foreground text-xs">{shipmentData.estimatedTime}</div>
          </div>
        </div>

        <div className="flex flex-1 flex-col">
          {shipmentData.timeline.map((item, index) => (
            <div className="flex items-start gap-3" key={`${item.company}-${item.time}`}>
              <div className="relative flex flex-col items-center">
                <div className={cn("z-[1] h-3 w-3 rounded-full border-2", getTimelineStatus(item))} />
                {index < shipmentData.timeline.length - 1 && <div className="mt-0.5 h-12 w-px bg-border" />}
              </div>
        ...
        <div className="flex items-center gap-2">
          <Button className="flex h-auto items-center gap-1.5 px-3 py-1.5 text-xs" size="sm" variant="outline">
            <Phone size={16} />
            Call
          </Button>
          <Button className="flex h-auto items-center gap-1.5 px-3 py-1.5 text-xs" size="sm" variant="outline">
          ...
          <Button className="ml-auto h-auto px-2 py-1.5 text-muted-foreground text-xs" size="sm" variant="ghost">
            View Details
          </Button>
        </div>
      </Card>
